...
async function _summarizeWithOllamaInternal(bodyText, title, publishedAt) {
  ...
  try {
    ...
    **문장 종결 규칙**: 모든 문장의 끝은 반드시 문맥에 맞는 **적절한 명사**로 자연스럽게 종결하라. 
    (예: 체결, 제공, 추진, 예정, 완료, 분석, 도입, 발표, 강화, 전망, 기대 중 문맥에 맞는 것 선택)
    
    주의사항:
    - '체결'과 같은 특정 단어를 모든 문장에 반복해서 붙이지 마라.
    - 문장의 의미가 통하는 자연스러운 명사를 스스로 선택하라.
    - '~함', '~임', '~다'로 끝나면 절대 안 된다.
    - 숫자가 앞에 붙는 불렛포인트(1.)를 절대 사용하지 마라.`;
    ...
    
    {
      ...
      "category": "분류 결과",
      "summary": [
        "핵심 내용을 담은 자연스러운 명사형 문장 1",
        "핵심 내용을 담은 자연스러운 명사형 문장 2",
        "핵심 내용을 담은 자연스러운 명사형 문장 3",
        "핵심 내용을 담은 자연스러운 명사형 문장 4"
      ],
      "published_at": "${publishedAt || new Date().toISOString().split('T')[0]}"
    }
    
    요약 가이드:
    1. 제목: 영문은 한글 번역, 매체명 제거.
    2. 카테고리: [AI, Robot, Security, Data, Display, IT, 기타] 중 택 1.
    3. 요약문: 문맥상 가장 어울리는 명사로 끝맺음. (예: ~기술 도입, ~계약 체결, ~성능 강화, ~일정 발표 등)
    4. 기사 발행일: "${publishedAt || '날짜 정보 없음'}" 참고.
...
<핵심 요약 명사형 문장 (최대 4개 작성, 문장 끝은 반드시 명사로 종결)>

주의사항:
- 반드시 '제목:'으로 시작하는 한국어 번역 제목 1줄과 숫자가 없는 **최대 4개**의 핵심 문장으로 작성하세요. 
- 각 문장의 끝은 반드시 '~함', '~했음' 대신 '체결', '제공', '추진', '완료', '예정' 등 **명사 그 자체로 종결**하세요.
