...
const RSS_FEEDS = [
  ...
  // 사내망 로컬 실행 시에는 정상 동작하지만 Oracle 운영 서버에서는 항상 실패함.
  // { name: "한국경제-IT", url: "https://www.hankyung.com/feed/it" },
  // 위 차단 우회를 위해 Google News 경유로 대체 (목록 조회는 정상, 요약 동작은 테스트 필요)
  { name: "한국경제-IT", url: "https://news.google.com/rss/search?q=when:24h+site:hankyung.com+(AI+OR+IT+OR+반도체+OR+기술)&hl=ko&gl=KR&ceid=KR:ko" },
  ...
  { name: "Bloomberg", url: "https://news.google.com/rss/search?q=when:24h+site:bloomberg.com+(AI+OR+Robot+OR+Security+OR+Tech)&hl=en-US&gl=US&ceid=US:en" },
  ...
  try {
    ...
    **가장 중요한 규칙**: 각 문장의 끝은 반드시 ~함, ~임 대신에 **"~입니다", "~했습니다"**와 같은 **자연스러운 평어체**로 종결하라.
    ...
    ${isEnglish ? '- title: 원본 영어 제목을 반드시 한국어로 번역하세요.\n    ' : ''}- category: [AI, Robot, Security, Data, Display, IT, Energy, 기타] 중 하나 선택.
      * Energy: 신재생에너지(태양광, 풍력, 수소 등), 히트펌프, 이차전지/배터리, 전력망, RE100, 기후기술, 친환경 에너지 솔루션/금융 관련 뉴스
      * AI: AI, LLM, 머신러닝, 생성형 AI 관련 뉴스
      * Robot: 로보틱스, 자율주행, 휴머노이드 관련 뉴스
      * Security: 보안, 해킹, 정보보호 관련 뉴스
      * Data: 빅데이터, 클라우드, 데이터센터 관련 뉴스
      * Display: 디스플레이, OLED, 패널 관련 뉴스
      * IT: 반도체, 스마트폰, 텔레콤, IT/테크 일반 뉴스
      * 기타: 위 7개 범주에 해당하지 않는 뉴스
