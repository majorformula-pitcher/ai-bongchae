...
async function _summarizeWithOllamaInternal(bodyText, title, publishedAt) {
  ...
  try {
    ...
    각 문장 사이에는 줄바꿈(\\n)을 넣어라.
    각 문장은 자연스러운 문장체(~이다, ~합니다, ~했습니다 등)를 사용하라.
    ...
    
    {
      ...
      "category": "분류 결과",
      "summary": [
        "핵심 내용을 담은 자연스러운 문장 1",
        "핵심 내용을 담은 자연스러운 문장 2",
        "핵심 내용을 담은 자연스러운 문장 3",
        "핵심 내용을 담은 자연스러운 문장 4"
      ],
    ...
    - summary: 핵심 내용을 요약하여 최대 4개의 자연스러운 문장을 작성하세요. "~이다" 또는 "~합니다" 체를 사용하세요. 1., 2. 같은 숫자를 문장 앞에 절대 넣지 마세요.
...
async function summarizeWithGemini(bodyText, title, publishedAt) {
  ...
  const prompt = `
    ...
    - 자연스럽고 매끄러운 문장체(~이다, ~합니다 등)를 사용하여 요약하세요.
...
- 마크다운 문법을 절대 사용하지 마세요. 1., 2. 같은 숫자를 붙이지 마세요.
  ...
  - 자연스럽고 매끄러운 문장체(~이다, ~합니다 등)를 사용하세요.
