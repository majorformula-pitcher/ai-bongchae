...
async function summarizeForPublish(title, summary) {
  ...
  1. 글자 수 제한:
     ...
     - "summary_ko": 공백 포함 **각 문장당 45자 이상 55자 이내**의 요약 문장 2개를 담은 배열. (절대 60자를 넘지 말 것)
  2. 말투 및 형식:
  ...
  [작성 예시 - 공백 포함 60자를 절대 넘지 않은 모범 작성 예시]
  {
    "title_ko": "지멘스와 영국 휴머노이드사의 에를랑엔 공장 로봇 테스트 완료",
    "summary_ko": [
      "독일 지멘스가 영국 휴머노이드사와 협력하여 물류 효율화를 위한 로봇 주행 테스트 성공",
      "인공지능 인프라와 첨단 시뮬레이션 도구를 적극 활용하여 자율 주행 성능 최적화"
    ]
  }

  [제약 조건 엄수 요청]
  - 문장 길이는 반드시 공백 포함 **45자~55자** 사이로 작성하여 PPT 상에서 정확히 2줄을 넘지 않도록 해야 합니다.
  - **단 1글자도 공백 포함 60자를 절대 초과해서는 안 됩니다.**

  - "title_ko": 공백 포함 **반드시 25자 이상 30자 이내**
  - "summary_ko": 공백 포함 **각 문장당 반드시 45자 이상 55자 이내** (최대 60자)
